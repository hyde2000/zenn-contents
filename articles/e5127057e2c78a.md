...
| フィールド | 説明                                              |
| ---------- | ------------------------------------------------- |
| program_id | Program の公開鍵。どの Program が実行されたか判別 |
| accounts   | 関連すべき Account を並べたメタデータ             |
| data       | Program に渡されるデータ(実装の中身)              |

つまりこれらをまとめると、

- Instrution は Program 内の実行ロジックの単位
- `program_id`が渡されるので、どの Program が呼ばれたかを判別できる
- コントラクトの実行により、ステートを管理するために Account を操作することができる
- クライアントから RPC で実行可能
